...
# this function searches for the dipole moment in the cccbdb webpage. the calculated dipole value we choose is calculated with the hartree fock method with the 6-31G* basis set
# the function inputs the chemical formula of the compound 
# the function outputs the dipole moment as a float if found and "None" if the dipole moment is not found
def get_dipole_moment(formula): # define the function

    print(f"Checking dipole moment for molecule: {formula}") # the cccbdb webpage is quite slow. This allows to track the search process when dealing with multiple moelcular formulas
    ...
    headers = {
        'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8', # tells the server what type of content we want, in this case: HTML,XHTML and XML
        ...
        'referer': 'https://cccbdb.nist.gov/dipole1x.asp', # specifies the full URL from which the request is being made
    ...
    try: # the server info request may fail, the try allows to handle the cases where it works and the exceptions allow to handle cases when it does not work
        ...
        session.get('https://cccbdb.nist.gov/dipole1x.asp') # request information to the dipole calculation page of the cccbdb
        ...
        dipole_response = session.get(f'{base_url}/dipole2x.asp', headers=headers) # here the url from which the data want to be get and the headers we used for the initial request
        
        # Parse the HTML to be able to search in it
        soup = BeautifulSoup(dipole_response.text, 'html.parser') # we parse the text (. text) of the info that the server sent us with the get
        
        # the dipole information we want is stored in the so called "table2". this I checked by doing a search in the browser and then viewing the page source 
        ...
        if table2: # if table 2 exists
            ...
            if hf_th: # in case a we find it
                ...
                hf_td = hf_th.find_next('td', class_='num bordered') # this searches for the next table data cell after hf_th that is from a class 'num bordered'
                ...
                if hf_td and hf_td.a:
                    ...
                    try:
                        ...
                        print(f"Dipole moment for {formula}: {value}") # prints the output of the dipole moment
...
def filter_molecules_by_dipole(input_csv, output_csv_good, output_csv_no_value, output_csv_discarted, output_csv_joined, delay=1): # function definition
    ...
    for index, row in df.iterrows(): # iterate through the rows of the csv file
        ...
        try: # it may not be possible to get the dipole moment from the server so we use "try"
            ...
            dipole_moment = get_dipole_moment(formula) # call the get_dipole_moment function
